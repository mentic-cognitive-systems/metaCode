In updateing this document it became apparent that we needed to rethink the proposed archecture. The original idea was to decompose MVC into static and dynamic components, but we hadn't deconstructed the problem fully; we had missing components; just like MVC does. Eventually we evolved WHAM into the AXIS.  This document is being left here as a reference for the evolution of our thinking, but the current recommended architecture is AXIS, which has clearer separation of concerns and more intuitive component roles.
...
The instructions as structured are the Static State. So with that you have complete mental modal for structuring your static and dynamic logic;
...
It is these components; W,Ɐ,I,I,I,A,M and their relationships that spell out the WHAM architecture.
...
- immutable state snapshots, not live values
